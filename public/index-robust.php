<?php
/**
 * Alternative index.php für Shared Hosting
 * Diese Version versucht automatisch die korrekten Pfade zu finden
 */

error_reporting(E_ALL);
ini_set('display_errors', 0); // Auf Produktionsserver ausgeschaltet

// Verschiedene mögliche Pfade für die Bootstrap-Datei testen
$possibleBootstrapPaths = [
    __DIR__ . '/../config/bootstrap.php',      // Standard-Pfad
    __DIR__ . '/config/bootstrap.php',         // Wenn config im public liegt
    __DIR__ . '/../airbnb-guest-system/config/bootstrap.php', // Wenn anders strukturiert
    $_SERVER['DOCUMENT_ROOT'] . '/../config/bootstrap.php',   // Über Document Root
    $_SERVER['DOCUMENT_ROOT'] . '/config/bootstrap.php',      // Config in Document Root
];
...
if (!$bootstrapPath) {
    ...
    echo '</ul>';
    echo '<p>Please check your file structure and paths.</p>';
    echo '</body></html>';
    exit;
}
...
try {
    ...
    echo '<p><strong>Error loading bootstrap:</strong> ' . htmlspecialchars($e->getMessage()) . '</p>';
...
try {
    ...
    $encryptionKey = envRequired('GUEST_ACCESS_ENCRYPTION_KEY');
} catch (Exception $e) {
    header('HTTP/1.1 500 Internal Server Error');
    echo '<!DOCTYPE html><html><head><title>Configuration Error</title></head><body>';
    echo '<h1>Configuration Error</h1>';
    echo '<p><strong>Missing environment variable:</strong> ' . htmlspecialchars($e->getMessage()) . '</p>';
    echo '<p>Please check your .env file configuration.</p>';
    echo '</body></html>';
    exit;
}

// Basic Auth Schutz für Admin-Bereich
$adminUser = envOr('ADMIN_USERNAME', 'admin');

// Basic Auth Header extrahieren (funktioniert mit verschiedenen Webservern)
$authUser = null;
$authPass = null;

if (isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW'])) {
    // Standard PHP Basic Auth
    $authUser = $_SERVER['PHP_AUTH_USER'];
    $authPass = $_SERVER['PHP_AUTH_PW'];
} elseif (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    // Authorization Header parsen
